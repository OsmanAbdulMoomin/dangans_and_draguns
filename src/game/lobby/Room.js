...
const Room = (props) => {
	...
	useEffect(() => {
		socketRef.current.on("lobbyFull", () => {
			alert("Lobby is full");
			history.push("/");
		});
		//create WebSocket
		...
		return () => {
			socketRef.current.disconnect("left");
		};
	}, [roomname]);
	...
	return (
		<Grid
			...
			container
			xs={12}
			style={{ border: "2px solid red", height: height }}
		>
			<Grid
				id="social-interaction-container"
				item
				container
				xs={4}
				style={{ border: "2px solid yellow", height: height }}
			>
				<Grid
					id="room-player-message-container"
					direction="column"
					item
					container
					xs={12}
					style={{ border: "2px solid green", height: height }}
				>
					<Grid
						item
						xs={2}
						style={{ border: "2px solid pink", maxWidth: "100%" }}
						zeroMinWidth
					>
						<Box flexGrow={1}>
